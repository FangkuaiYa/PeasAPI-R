...
namespace PeasAPI.Managers.UpdateTools
{
    public abstract class UpdateListener
    {
        ...
        public virtual void UpdateMod()
        {
            ...
            {
                using var httpClient = new HttpClient();
                httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("PeasAPI Updater");
        
                switch (Type)
                {
                    case FileType.Dll:
                        var dllBytes = httpClient.GetByteArrayAsync(AssetLink).Result;
                        File.WriteAllBytes($"{Name}.dll", dllBytes);
                        File.Move($"{Name}.dll", assemblyPath);
                        break;
            
                    case FileType.Zip:
                        var zipBytes = httpClient.GetByteArrayAsync(AssetLink).Result;
                        File.WriteAllBytes($"{Name}.zip", zipBytes);
            ...
            {
                PeasAPI.Logger.LogError($"Update failed: {ex.Message}");
            }
        }
    }
}